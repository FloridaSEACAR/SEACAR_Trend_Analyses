...
sav_results$`Period of Record` <- paste0(sav_results$EarliestYear, " - ", sav_results$LatestYear)
...
for(ma in unique(sav_results$ManagedAreaName)){
  ...
  cat(glue("# {ma}"))
  cat("  \n")
  if(nrow(sav_results[ManagedAreaName==ma & SufficientData==TRUE, ])==0){
    cat("  \n")
    cat(glue::glue("Not enough data to perform analyses in {ma}"))
    cat("  \n")
  }
  ma_abrev <- MA_All[ManagedAreaName==ma, Abbreviation]
  for(plot_type in plot_types){
    fig_caption <- figureCaptions[HabitatName=="Submerged Aquatic Vegetation" & 
                                    IndicatorName=="Percent Cover" & 
                                    PlotType==plot_type, FigureCaptions]
    ...
    if(plot_type=="multiplots"){
      ...
      print(display_table)
      cat("  \n")
      desc <- descriptionTable[ManagedAreaName==ma & HabitatName=="Submerged Aquatic Vegetation", get(descriptionColumn)]
      cat(desc)
      cat("  \n")
    }
  }
