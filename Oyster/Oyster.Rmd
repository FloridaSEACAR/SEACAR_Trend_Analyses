...
```{r, functions to plot oysters, eval=T, echo=F, warning=FALSE}
library(glue)
library(kableExtra)
# Read in oyster results file
oyster_stats <- fread("output/ManagedAreaName/Oyster_All_GLMM_Stats.txt", sep='|')
...
oy_figs <- list(
  "Density" = list.files("output/ManagedAreaName/Figures/Density/", pattern = ".png", full.names = T),
  "Percent Live" = list.files("output/ManagedAreaName/Figures/Percent_Live/", pattern = ".png", full.names = T),
  "Shell Height" = list.files("output/ManagedAreaName/Figures/Shell_Height/", pattern = ".png", full.names = T)
)
...
oyster_includes <- oyster_includes[!(HabitatType=="Restored" & ManagedAreaName %in% c("Apalachicola Bay Aquatic Preserve", "Apalachicola National Estuarine Research Reserve") & ParameterName=="Shell Height")]
oyster_includes <- oyster_includes[!(HabitatType=="Natural" & ManagedAreaName %in% c("Tomoka Marsh Aquatic Preserve") & ParameterName=="Shell Height")]
...
oyster_tables <- function(ma, p, type){
  ...
  # Determine which columns to select for display within report tables (shell height includes additional size class column)
  if(p=="Shell Height"){
    ResultTable <- subset %>% 
      filter(!SizeClass=="") %>%
      dplyr::select(ShellType, HabitatType, SizeClass, "Trend Status", ModelEstimate, StandardError,"Credible Interval") %>%
    ...
    ResultTable <- subset %>% 
      dplyr::select(ShellType, HabitatType, "Trend Status", ModelEstimate, StandardError,"Credible Interval") %>%
      dplyr::rename("Shell Type" = "ShellType",
                    "Habitat Type" = "HabitatType",
                    "Estimate" = "ModelEstimate",
                    "Standard Error" = "StandardError")
...
tableDesc <- fread("output/oyster_tableDescriptions.csv")
```

```{r, Oyster figures, eval=T, echo=F, results='asis', warning=FALSE}
...
for(p in unique(oyster_includes$ParameterName)){
  ...
  for(ma in unique(p_subset$ManagedAreaName)){
    ...
    desc <- tableDesc[ManagedAreaName==ma & HabitatName=="Oyster/Oyster Reef" & ParameterName==p, get(descriptionColumn)]
    cat(glue("## {ma}"))
    cat("  \n")
    cat(desc)
    cat("  \n")
...
\newpage
# Spatio-temporal scope plots

```{r, spatio-temporal, eval=T, echo=F, results='asis', warning=FALSE}
scope_plot_locs <- list.files("output/Oyster_temporal_scope_plots/", pattern = ".jpg", full=T)

for(ma in unique(oyster_includes$ManagedAreaName)){
  cat(glue("## {ma}"))
  ma_abrev <- MA_All[ManagedAreaName==ma, Abbreviation]
  scope_plot <- str_subset(scope_plot_locs, paste0("_", ma_abrev, "_"))
  fig_caption <- paste0("Spatio-temporal availability of data in ", ma, ".")
  subchunkify(cat("![", fig_caption, "](", scope_plot,")"))
  cat("  \n")
}
...
# Plotting setup
```{r plotting_setup, message=FALSE, eval=F, echo=T}
# LiveDate Threshold -----------------------------------------------------
