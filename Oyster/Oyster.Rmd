...
```{r, functions to plot oysters, eval=T, echo=F, warning=FALSE, message=FALSE}
library(glue)
library(kableExtra)
...
oyster_stats <- oyster_stats %>% rowwise() %>%
  mutate("Credible Interval" = ifelse(!is.na(LowerConfidence), paste0(round(LowerConfidence, 2), " to ", round(UpperConfidence, 2)), NA)) %>%
...
oyster_tables <- function(ma, p, type){
  ...
  if(p=="Shell Height"){
    ResultTable <- subset %>% 
      filter(!SizeClass=="") %>%
      dplyr::select(ShellType, HabitatType, SizeClass, TrendText, ModelEstimate, StandardError,"Credible Interval") %>%
    ...
    ResultTable <- subset %>% 
      dplyr::select(ShellType, HabitatType, TrendText, ModelEstimate, StandardError,"Credible Interval") %>%
      dplyr::rename("Shell Type" = "ShellType",
                    "Habitat Type" = "HabitatType",
                    "Estimate" = "ModelEstimate",
                    "Standard Error" = "StandardError",
