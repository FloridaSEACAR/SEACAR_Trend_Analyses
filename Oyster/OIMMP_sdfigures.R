...
files <- list.files("C:/Users/Hill_T/Desktop/SEACAR GitHub/MonitoringPlanningTools/Oyster_Sample_Size_Site_Selection/Updated_Model_Workflow/data", pattern = "shiny_store", full=T)

loc_files <- function(oimmp, p){stringr::str_subset(files, paste0(gsub(" ", "_", oimmp), "_", gsub(" ", "_", p)))}

for(oimmp in unique(oyster$OIMMP)){
  if(oimmp == "Northeast Florida") next
  cat("-", oimmp, "\n")
  for(p in c("Density", "Shell Height", "Percent Live")){
    ...
    ##### Parameter SD maps -----
    pal <- colorBin("YlOrRd", datawizard::normalize(shiny_store$oyster.oimmp_parameter_positions_sd$sd))
    ...
    map <- leaflet() %>% 
      ...
      addCircleMarkers(lng = shiny_store$oyster.oimmp_parameter_positions_sd$X,
                       lat = shiny_store$oyster.oimmp_parameter_positions_sd$Y,
                       color = pal(datawizard::normalize(shiny_store$oyster.oimmp_parameter_positions_sd$sd)),
                       radius = 5,
                       options = pathOptions(pane = "foreground")) %>%
      addLegend(pal = pal, values = datawizard::normalize(shiny_store$oyster.oimmp_parameter_positions_sd$sd),
      ...
      fitBounds(lng1 = max(shiny_store$oyster.oimmp_parameter_positions_sd$X),
                lat1 = max(shiny_store$oyster.oimmp_parameter_positions_sd$Y),
                lng2 = min(shiny_store$oyster.oimmp_parameter_positions_sd$X),
                lat2 = min(shiny_store$oyster.oimmp_parameter_positions_sd$Y)) %>%
      addControl(paste("Oyster", tolower(p), "standard deviation on sampled reefs in", oimmp, "- ExportDate:", exportDate), position="bottomleft", className="map-title")
...
for(oimmp in unique(reef_data_overview$OIMMP)){
  ...
  ggsave(reef_plot, path = paste0(output_path, "ReportFigures/", 
                                  gsub(" ", "_", oimmp), "_reef_overview.png"), height = 6, width = 10)
}