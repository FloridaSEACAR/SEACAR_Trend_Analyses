...
This markdown file is designed to be compiled by [Coral_Plot_Render.R](https://github.com/FloridaSEACAR/SEACAR_Trend_Analyses/blob/main/Coral/Coral_Plot_Render.R) (https://github.com/FloridaSEACAR/SEACAR_Trend_Analyses/blob/main/Coral/Coral_Plot_Render.R).

\newpage
# Coral Percent Cover

```{r show_plots, results='asis', echo=FALSE, warning=FALSE}
...
coral_lme <- fread("output/PercentCover/Coral_PC_LME_Stats.txt")
...
for(ma in sort(unique(coral_lme$ManagedAreaName))){
  fig_caption <- figureCaptions[HabitatName=="Coral/Coral Reef" & IndicatorName=="Percent Cover", FigureCaptions]
  desc <- descriptionTable[ManagedAreaName==ma & HabitatName=="Coral/Coral Reef" & ParameterName=="Percent Cover", get(descriptionColumn)]
  cat("  \n")
  cat(glue("## {ma}"))
  ...
  cat("  \n")
  SEACAR::subchunkify(make_table(ma, tableFormat))
  cat("  \n")
  cat(desc)
  cat("  \n")
  map_loc <- get_map(ma)
  SEACAR::subchunkify(cat("![](", map_loc,")"))
  cat("  \n")
}
