...
plot_coral_pc <- function(ma, ma_abrev, lme_plot = lme_plot_pc, coral_pc_plots, report_type){
  ...
  ResultTable <- ResultTable %>% rowwise() %>%
    mutate(`Statistical Trend` = TrendText,
           `Period of Record` = paste0(EarliestYear, " - ", LatestYear)) %>% 
    ungroup() %>% 
    rename(`LME Intercept` = LME_Intercept, `LME Slope` = LME_Slope, 
           `p` = LME_p) %>% 
  ...
  desc <- TableDescriptions[ManagedAreaName==ma & HabitatName=="Coral Reef" & ParameterName=="Percent Cover", get(descriptionColumn)]
  # Table title
  ...
  fig_caption <- FigureCaptions[HabitatName=="Coral Reef" & IndicatorName=="Percent Cover", FigureCaptions]
...
plot_coral_sr <- function(ma, ma_abrev, MA_Ov_Stats = MA_Ov_Stats_sr, coral_sr_plots, report_type){
  ...
  ResultTable <- ResultTable %>%
    ...
    select(N_Data, N_Years, `Period of Record`, Median, Mean) %>%
    rename(`No. of Samples` = N_Data, `No. Years with Data` = N_Years, 
           `Median No. of Taxa` = Median, `Mean No. of Taxa` = Mean) %>% 
    as.data.table()
  ...
  # Grab relevant table description for a given plot
  desc <- TableDescriptions[ManagedAreaName==ma & HabitatName=="Coral Reef" & ParameterName=="Presence/Absence", get(descriptionColumn)]
  # Table title
  ...
  fig_caption <- FigureCaptions[HabitatName=="Coral Reef" & IndicatorName=="Grazers and Reef Dependent Species", FigureCaptions]
  subchunkify(cat("![", fig_caption, "](", plot_loc,")"))
  # cat("![](", plot_loc,")")
  cat("  \n")
  print(result_table)
