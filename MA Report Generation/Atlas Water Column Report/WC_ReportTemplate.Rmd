...
load_data_table <- function(p, a="All", d="All", table="data") {
  
  # Declaring RDS file list of respective tables
  files <- list.files(paste0(data_obj_loc,"disc/", disc_folder_date, "/"),pattern = "\\.rds$")
  ...
  table_file <- paste0(paste0(data_obj_loc,"disc/", disc_folder_date, "/"),str_subset(files, filename_string))
...
``` {r load_cont_data_table function, eval=FALSE}
# For loading continuous data
# Load Data Table Function
load_cont_data_table <- function(p, table) {
  
  # Declaring RDS file list of respective tables
  files <- list.files(paste0(data_obj_loc,"cont/",cont_folder_date,"/"),pattern = "\\.rds$")
  
  # subset file list to select desired table RDS file
  table_file <- paste0(data_obj_loc,"cont/",cont_folder_date,"/",str_subset(files, table))
...
plot_trendlines <- function(p, a, d, activity_label, depth_label, y_labels, parameter, skt_data, discrete_data) {
  ...
  if (nrow(data) == 0) {
    ...
    ggsave(filename = paste0("output/WQ_Discrete/", ma_short, "_", 
           ...
           plot = p1, width = 1600, height = 1142, units = "px", dpi = 300,
           scale = 2)
    
    # print(xtable(ResultTable,
    #              caption = paste0("Seasonal Kendall-Tau Results for - ", parameter),
    #              tabular.environment = "longtable", scalebox = 0.7))
...
plot_trendlines_cont_combined <- function(param, y_labels, parameter){
  skt_stats <- skt_stats_cont %>% 
    filter(ManagedAreaName==ma & ParameterName==parameter)
  
  data <- setDT(data_output_cont[[param]] %>% filter(ManagedAreaName==ma))
  ...
  if(length(unique(data$ProgramLocationID))>0){
    ...
    ### SKT STATS ###
    # Gets x and y values for start and end points for trendline
    KT.Plot <- skt_stats %>%
    ...
    if(length(unique(data$ProgramLocationID))>10){
      ...
      for(pid in unique(data$ProgramID)){
        ...
        # all plots together
        plot_data <- merge(
          data[data$ManagedAreaName==ma & ProgramID==pid,], 
          KT.Plot[ProgramID==pid, ], 
          by=c("ProgramID", "ProgramLocationID", "MonitoringID"), 
          all=TRUE)
        
        plot_data <- plot_data %>% mutate(label = paste0(ProgramLocationID, " - ", RelativeDepth))
        setDT(plot_data)
      ...
      # all plots together
      plot_data <- merge(data, KT.Plot, by=c("ProgramID", "ProgramLocationID", "MonitoringID"), all=TRUE)
      plot_data <- plot_data %>% mutate(label = paste0(ProgramLocationID, " - ", RelativeDepth))
      setDT(plot_data)
      
      plot_data <- merge(plot_data, skt_stats[,c("p","MonitoringID")], by=c("MonitoringID"))
      plot_data[, `:=` (sig = ifelse(p<=0.05, "Significant Trend", "Non-significant Trend"))]
...
for(indicator in unique(websiteParams$IndicatorName)){
  ...
  for(i in 1:nrow(indicator_subset)){
    ...
    if(type=="Continuous"){
      plot_trendlines_cont_combined(param_short, y_labels, parameter)
    }
