...
get_plot <- function(ma_short, parameter, type, pid){
  ...
  if(type=="Discrete"){
    pattern <- paste0("output/WQ_Discrete/",ma_short, "_", gsub(",","",(gsub(" ","_",parameter))), ".png")
    file <- str_subset(disc_plots, pattern)
  }else{
    # When all stations are plotted without ProgramID subdivisions
    if(pid=="none"){
      pattern <- paste0("output/WQ_Continuous/",ma_short, "_", gsub(",","",(gsub(" ","_",parameter))), ".png")
    # When parameter data is divided into ProgramID subdivisions
    } else {
      pattern <- paste0("output/WQ_Continuous/",ma_short, "_", gsub(",","",(gsub(" ","_",parameter))), "_", pid, ".png")
    }
  ...
  if(length(unique(data$ProgramLocationID))>0){
    ...
    if(length(unique(data$ProgramLocationID))>10){
      for(pid in unique(data$ProgramID)){
        ...
        ResultTable$ProgramLocationID <- gsub("_", "-", ResultTable$ProgramLocationID)
        ResultTable$ProgramLocationID <- gsub("&", "-and-", ResultTable$ProgramLocationID)
        ...
        cat("  \n")
        # Display plot
        plot_loc <- get_plot(ma_short = ma_short, parameter = parameter, type = type, pid = pid)
        # include_graphics() does not work within loop, force markdown format
        cat("![](", plot_loc,")")
        cat("  \n")
        print(result_table)
