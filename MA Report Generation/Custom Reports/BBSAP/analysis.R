...
for(i in 1:length(all_params)){
  ...
  for(depth in all_depths){
    ...
    for(activity in all_activity){
      ...
      data <- merge.data.frame(data, MA_Summ[,c("System", "SufficientData")],
                               by="System")
      ...
      MA_Summ <- MA_Summ %>%
        select(System, ParameterName, RelativeDepth, ActivityType,
      ...
      c_names <- c("System", "Independent", "tau", "p",
      ...
      if(n==0){
        ...
        for (i in 1:n) {
          ...
          data_SKT <- YM_Stats[YM_Stats$System==sys_include[i], ]
          x <- nrow(data_SKT)
          # Perform analysis if there is more than 1 row
          if (x>0) {
            # Store the managed area summary statistics to be used in
            # trend analysis
            SKT.med <- MA_Summ$Median[MA_Summ$System==sys_include[i]]
            SKT.minYr <- MA_Summ$EarliestYear[MA_Summ$System==sys_include[i]]
            SKT.maxYr <- MA_Summ$LatestYear[MA_Summ$System==sys_include[i]]
            SKT.ind <- TRUE
            ...
            skt_stats$System[i] <-
              MA_Summ$System[MA_Summ$System==sys_include[i]]
