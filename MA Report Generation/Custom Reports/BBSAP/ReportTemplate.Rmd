...
```{r maps, results='asis', eval=TRUE, dev='png', dpi=300}
# WQ MAP
cat("  \n")
cat("\\newpage")
cat("# Maps")
...
cat("  \n")
SEACAR::subchunkify(sav_maps_list[["all_sys_map"]], 8, 8)
cat("  \n")
...
cat("  \n")
SEACAR::subchunkify(sav_maps_list[["all_sys_type_map"]], 8, 8)
cat("  \n")
...
cat("  \n")
SEACAR::subchunkify(sav_maps_list[["all_sys_sav_map"]], 8, 8)
cat("  \n")
cat("  \n")

```

```{r system_plots, results='asis', eval=TRUE, fig.height=11, fig.width=7}

for(sys in unique(wq_data$System)[1]){
# for(sys in unique(wq_data$System)){
  ...
  cat(sys_subtitle, "\n\n")
  cat("  \n")
  SEACAR::subchunkify(sav_maps_list[["wq_sys_maps"]][[sys]], 8, 8)
  cat("  \n")
  ...
  # Display table
  SEACAR::subchunkify(make_kable(
    system_overview %>% arrange(ParameterName),
  ...
  SEACAR::subchunkify(
    kbl(
      prog_data[System==sys, -c("System", "ProgramName")],
      format="latex", 
  ...
  for(param in unique(data_combined[System==sys, ParameterName])){
    ...
    cat(param_subtitle, "\n\n")
    cat("  \n")
    SEACAR::subchunkify(plot_trendlines(sys, param, activity_label, depth_label, y_labels), 7, 11)
    cat("  \n")
  }
  ...
  if(sys %in% unique(SAV4$System)){
    ...
    SEACAR::subchunkify(sav_maps_list[["sav_sys_maps"]][[sys]], 8, 8)
    ...
    for(plot_type in plot_types){
      ...
      cat(subtitle, "\n\n")
      cat("  \n")
      SEACAR::subchunkify(sav_plots(sys, plot_files, plot_type), 9, 10)
      cat("  \n")
      if(plot_type=="trendplot"){
        SEACAR::subchunkify(sav_trend_table(sys))
        cat("  \n")
      }
    }
    cat("  \n")
...
```{r SAV_plots_all_systems, results='asis', fig.height=9, fig.width=10, eval=TRUE}
cat("  \n")
cat("\\newpage")
cat("  \n")
