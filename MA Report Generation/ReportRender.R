...
folder_paths <- c("output", "output/Reports", "output/maps",
                  "output/Reports/HTML", "output/Reports/PDF")
...
files <- list.files(paste0(seacar_data_location, "/archive/2025-Mar-06"), full.names=TRUE)
...
species_available <- function(ma){
  ...
  if(in_sav){
    dataFile <- fread(sav_file_in, sep='|', na.strings = "NULL") %>%
  ...
  if(in_coral){
    dataFile <- fread(coral_file_in, sep='|', na.strings = "NULL") %>%
      filter(ManagedAreaName==ma)
    all_sp <- c(all_sp, unique(dataFile$CommonIdentifier))
  ...
  if(in_cw){
    dataFile <- fread(cw_file_in, sep='|', na.strings = "NULL") %>%
      filter(ManagedAreaName==ma)
    all_sp <- c(all_sp, unique(dataFile$CommonIdentifier))
  ...
  if(in_nekton){
    dataFile <- fread(nekton_file_in, sep='|', na.strings = "NULL") %>%
      filter(ManagedAreaName==ma)
    all_sp <- c(all_sp, unique(dataFile$CommonIdentifier))
...
FigureCaptions <- openxlsx::read.xlsx("data/AtlasFigureCaptions_Final.xlsx") %>% 
...
TableDescriptions <- openxlsx::read.xlsx("../../QAQC-Tools/TrendText/tableDescriptions/output/Atlas_Descriptions_2025-05-15_Final.xlsx") %>%
