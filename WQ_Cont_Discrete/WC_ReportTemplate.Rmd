...
if(file_type=="PDF"){
  descriptionTable <- descriptionTable %>% 
    mutate(DescriptionLatex = stringi::stri_replace_all_regex(
      DescriptionLatex,
      pattern = c("<p>", "</p>", "&deg;C", "&micro;"),
      replacement = c("", "", "Deg C", "ug"),
      vectorize = FALSE
    )) %>% as.data.table()
}
...
plot_discrete <- function(p, a, d, parameter, skt_data, discrete_data, ma, ma_short, type, indicator_short) {
  ...
  if (nrow(data) == 0) {
    ...
    # Get table description text for table caption
    desc <- descriptionTable[ManagedAreaName==ma & ParameterName==parameter & SamplingFrequency=="Discrete", get(descriptionColumn)]
    
    # Creates ResultTable to display statistics below plot
    ResultTable <- skt_stats %>%
      select(ActivityType, TrendText, N_Data, N_Years, "Period of Record", 
             Median, tau, SenIntercept, SenSlope, p) %>%
      rename("Activity Type" = ActivityType,
             "Sample Count" = N_Data,
             "Years with Data" = N_Years,
             "Median Result Value" = Median,
             "Tau" = tau, "Sen Intercept" = SenIntercept, "Sen Slope" = SenSlope, "P" = p,
    ...
    SEACAR::subchunkify(result_table)
    cat("  \n")
    cat("\n \n \n")
    ...
    if(length(unique(data$ProgramLocationID))>10){
      for(prog_n in 1:length(unique(data$ProgramID))){
        ...
        ResultTable <- skt_stats %>%
          ...
          select(ProgramLocationID, TrendText, N_Data, N_Years, "Period of Record", Median, tau,
        ...
        # Get table description text for table caption
        desc <- descriptionTable[ManagedAreaName==ma & ParameterName==parameter & SamplingFrequency=="Continuous", get(descriptionColumn)]
